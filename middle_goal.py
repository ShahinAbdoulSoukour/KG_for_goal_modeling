...
async def middle_goal(request: Request, goal_name: str = Form(...), goal_type: str = Form(...), hlg_id: int = Form(...), db: Session = Depends(get_db)):
    ...
    # Get the current refinement from the current hierarchy
    current_refinement = db.query(models.Hierarchy.refinement).filter(
    ...
    new_hierarchy_high = models.Hierarchy(refinement=current_refinement[0], high_level_goal_id=hlg_id, subgoal_id=new_middle_goal.id)
    db.add(new_hierarchy_high)

    for subgoal in subgoals:
        # New hierarchy between new middle goal and the existing subgoals
        new_hierarchy_sub = models.Hierarchy(refinement=current_refinement[0], high_level_goal_id=new_middle_goal.id, subgoal_id=subgoal.id)
